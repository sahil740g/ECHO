...
function Profile() {
  ...
  if (!username) {
    ...
    // Check if param matches logged-in user's handle
  ...
  const handleMessage = async () => {
    ...
    if (!displayUser || !displayUser.id) {
      console.error("Cannot message user without ID:", displayUser);
      return;
    }

    // Use displayUser.id instead of handle
    try {
      const chat = await getOrCreateChat(displayUser.id);
      if (chat) {
  ...
  useEffect(() => {
    if (user && displayUser && displayUser.handle) {
  ...
  if (!displayUser && username) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-zinc-400">
        ...
        <p>
          The user <span className="text-blue-400">@{username}</span> does not
          exist.
        </p>
        <button
            ...
            <div className="flex flex-col md:flex-row items-start justify-between gap-4 md:gap-0">
              <div className="flex gap-4 md:gap-6 items-start w-full md:w-auto">
                ...
                <div className="pt-1 md:pt-2 flex-1">
                  ...
                  <p className="text-zinc-500 text-sm mb-3 md:mb-4">
                    {profileData.handle}
                  </p>
                ...
                {userPosts.length > 0 ? (
                  ...
                  <div className="py-20 text-center">
                    ...
                    <p className="text-zinc-500 text-sm">
                      Posts created by {profileData.name} will appear here
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-4">
              {profileData.socials.x && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-3 text-zinc-400 hover:text-white transition text-sm"
                >
                  <X size={16} />X
                </a>
              )}
