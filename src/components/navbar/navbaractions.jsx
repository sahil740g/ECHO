import { PlusSquare, UserCircle, User, Bookmark, Settings, LogOut, Search, ArrowLeft, Bell } from "lucide-react";
      ...
      {!user ? (<button onClick={openLoginModal} className="bg-blue-600 text-white px-5 py-2 rounded-full text-sm font-medium hover:bg-white hover:text-black transition cursor-pointer">Log in</button>) : (
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 text-zinc-400 hover:text-white relative transition"
          >
            <Bell size={24} />
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-blue-500 rounded-full border border-[#0d1117] flex items-center justify-center">
              </span>
            )}
          </button>
